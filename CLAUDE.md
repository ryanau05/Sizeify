...
**Current status (as of 2026-06-11):** Phase 0 (Foundation) is complete and the repo is **private** (GitHub Pro) with branch protection intact. Phase 1 (Backend core) is **in progress**: schema, repositories, Pydantic schemas, auth (Argon2 + JWT), and migrations `0001`/`0002` are landed (commit `6c94ebb`, "Phase 1 backbone"); the domain core (`stretch`, `fit_profile`, `matching`, `recommendation`) and closet endpoints are **not built yet**. Active focus is the `demo/capstone` branch, where the demo layer is currently **scaffolding only** (structure exists, uncommitted; no DEMO ticket implemented beyond DEMO-00). See `docs/DEMO_TICKETS.md` to start implementing. Keep this paragraph current — if the date here is more than a week old, treat the status as suspect and re-read `docs/PROJECT_PLAN.md` and `docs/DEMO_TICKETS.md` before acting on it.
