...
**Capstone demo (parallel track):** `docs/DEMO_PLAN.md` and `docs/DEMO_TICKETS.md` describe a throwaway 3-day prototype on the `demo/capstone` branch (off `main`). It is deliberately isolated — a separate `DEMO_MODE=1` ASGI app (`api.demo.app:app`) plus a throwaway web client — and its domain-core tickets (DEMO-01–04) are *real* Phase 1 work pulled forward, to be cherry-picked back. `DEMO_TICKETS.md` has a progress tracker; consult it for demo status. Nothing on `demo/capstone` reaches `main` or production except deliberate cherry-picks.

**Current status (as of 2026-07-19):** Phase 0 (Foundation) is complete. The repo is **public** (deliberately — capstone reviewers need access without an invite), with branch protection on `main` intact: 3 required status checks, so `main` takes PRs only. Phase 1 (Backend core) is **in progress**: schema, repositories, Pydantic schemas, auth (Argon2 + JWT), and migrations `0001`/`0002` are landed (commit `6c94ebb`, "Phase 1 backbone"). The domain core (`stretch`, `fit_profile`, `matching`, `recommendation`) is **built and tested** — it was pulled forward on `demo/capstone` as DEMO-01–04 and is real Phase 1 work awaiting cherry-pick (TKT-P1-11/12/14/15). The roadmap closet endpoints are still unbuilt; the demo has its own throwaway versions. Active focus is `demo/capstone`, where both demo flows now run end-to-end against live Postgres. See `docs/DEMO_TICKETS.md` for per-ticket status — including three defects its earlier "Done" marks had hidden. Keep this paragraph current — if the date here is more than a week old, treat the status as suspect and re-read `docs/PROJECT_PLAN.md` and `docs/DEMO_TICKETS.md` before acting on it.
